...
const ALLOWED_USER = process.env.ALLOWED_TELEGRAM_USER;
...
- Si la demande est une question ou ne nécessite pas de modification de code, réponds normalement sans balises <file>`;
...
bot.on("message", async (msg) => {
  ...
  try {
    const claudeResponse = await askGroq(text);
    ...
    cloneAndPush(files, `bot: ${summary.slice(0, 72)}`);
...
console.log("🤖 CLC Traiteur Bot démarré (Groq)");
